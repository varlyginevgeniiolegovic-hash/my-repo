...
import {debugTriggerDownload, debugFontExport, addDebugUtilities} from '../helpers/fontExportDebugger.helpers';
...
function triggerDownload(arrayBuffer, filename) {
	console.log('🔽 Triggering font download:', filename);
	
	// Debug the font before download
	const debugInfo = debugFontExport(arrayBuffer, filename);
	
	if (!debugInfo.isValidFont) {
		console.warn('⚠️ Font validation failed, but proceeding with download for debugging');
		console.warn('Debug info:', debugInfo);
	}
	
	// Use the enhanced debug download function
	debugTriggerDownload(arrayBuffer, filename);
}
...
export default {
	...
	'/export-otf': async ({
		...
		// Check if template is available before proceeding
		if (!template) {
			console.warn('⚠️ Export: template is undefined, cannot export font');
			localClient.dispatchAction('/store-value', {
				export: false,
				errorExport: {message: 'Template not loaded, please try again'},
			});
			return;
		}
	...
	'/export-family-from-library': async ({
		...
		// Check if template is available before proceeding
		if (!template) {
			console.warn('⚠️ Export family: template is undefined, cannot export');
			localClient.dispatchAction('/exporting', {
				exporting: false,
				errorExport: {message: 'Template not provided for family export'},
			});
			return;
		}
