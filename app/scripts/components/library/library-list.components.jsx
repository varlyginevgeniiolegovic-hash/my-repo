...
class LibraryList extends React.Component {
	...
	componentDidUpdate(prevProps) {
		...
		if (
			...
			|| this.props.favourites !== prevProps.favourites
		) {
			this.generateFonts(
				this.props.families,
				this.props.presets,
		...
		if (
			...
			!== JSON.stringify(prevProps.librarySelectedTags)
		) {
			this.setState({librarySelectedTags: this.props.librarySelectedTags});
			this.filterFonts(
				this.state.activeFilters,
				this.props.librarySelectedTags,
				this.state.search,
				this.state.mode,
			);
		}
		if (this.props.location.query !== prevProps.location.query) {
			this.setState({
				mode: this.props.location.query && this.props.location.query.mode,
			});
			this.filterFonts(
				this.state.activeFilters,
				this.state.librarySelectedTags,
				this.state.search,
				this.props.location.query && this.props.location.query.mode,
			);
		}
	}
							...
							<button 
								...
								onClick={async () => {
									try {
										...
										console.log('Error refreshing library:', error);
									}
								}}
							>
								↻ Refresh Library
