...
export default class CanvasShadow extends React.PureComponent {
	...
	async componentDidMount() {
		...
		this.toile.setCamera(
			this.props.glyphViewMatrix.t,
			this.props.glyphViewMatrix.z,
			-height,
			width,
		);
		...
		*/
		...
		const rafFunc = () => {
			...
			this.toile.drawText(
				`${this.glyph}`,
				{x: 0, y: 0},
				1024 * this.props.glyphViewMatrix.z,
				'#fc5454',
				'shadowfont',
			);
			rafId = raf(rafFunc);
	...
	componentDidUpdate(prevProps) {
		const {glyphViewMatrix} = prevProps;
		const currentGlyphViewMatrix = this.props.glyphViewMatrix;

		console.log('new props');

		if (
			this.props.canvasMode === 'move'
			&& (glyphViewMatrix.t.x !== currentGlyphViewMatrix.t.x
				|| glyphViewMatrix.t.y !== currentGlyphViewMatrix.t.y)
		) {
			const delta = {
				x: currentGlyphViewMatrix.t.x - glyphViewMatrix.t.x,
				y: currentGlyphViewMatrix.t.y - glyphViewMatrix.t.y,
			};
			...
			case 'image':
				this.setState({
					...
					zoom: this.props.glyphViewMatrix.z,
				});
				break;
			case 'font':
				this.setState({
	...
	drawOnCanvas() {
		const zoom
			= this.props.canvasMode === 'move' && this.type === 'font'
				? this.state.glyphZoom
				: this.state.zoom;
		...
		const viewCenterY = (this.state.eyeY + viewH / 2 - srcHeight / 2).toFixed(
			2,
		);
	...
	onMouseMove(event) {
		if (
			...
			case 'image':
				this.setState({
					eyeX:
							this.state.lastMouseX === 0
								? this.state.lastMouseX
								: this.state.eyeX
									- (event.clientX - this.state.lastMouseX) / this.state.zoom,
					eyeY:
							this.state.lastMouseY === 0
								? this.state.lastMouseY
								: this.state.eyeY
									- (event.clientY - this.state.lastMouseY) / this.state.zoom,
