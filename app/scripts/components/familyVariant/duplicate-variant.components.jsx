...
export default graphql(getBaseValuesQuery, {
	...
	props({data, ownProps}) {
		...
		// Handle case where variant might be undefined (e.g., in offline mode)
		if (!data.variant) {
			console.warn('No variant data found, using original variant');
			return {variant: ownProps.variant};
		}
