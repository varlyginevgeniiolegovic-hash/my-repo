...
export const AddVariant = graphql(getBaseValuesQuery, {
	...
	props({data}) {
		...
		// Handle case where family or variants might be undefined (e.g., in offline mode)
		if (!data.family || !data.family.variants || data.family.variants.length === 0) {
			console.warn('No family or variants found, using fallback values');
			return {
				variantBase: {
					id: 'fallback-variant',
					values: {} // Empty values as fallback
				}
			};
		}
