...
class ArianneThread extends React.PureComponent {
	...
	render() {
		...
		// Filter out any null/undefined families and ensure we have valid data
		const validFamilies = (families || []).filter(f => f && f.name);
		...
		const family
			= validFamilies.find((f) => f.name === selectedFamily?.name) || validFamilies[0];
		const variant = (family && family.variants && family.variants.find(
			(v) => v && v.name === selectedVariant?.name,
		))
			|| (family && family.variants && family.variants[0]) || {name: 'regular'};
